...
## 二、交點計算
### 1. 兩直線交點
兩直線 $A_1x + B_1y = C_1$、$A_2x + B_2y = C_2$

克拉瑪公式，當$\Delta=0$時兩直線平行或重合線
...
- **計算圓心距**：$d = \sqrt{(h_2-h_1)^2 + (k_2-k_1)^2}$
...
### 從點到直線的垂直線
給定直線 $Ax + By + C = 0$ 和點 $P(x_0,y_0)$：

- **垂直線方程**：$Bx - Ay + (Ay_0 - Bx_0) = 0$
...
對於直角三角形：
- 垂直邊：$a = \text{distance}(P, \text{垂足})$
- 底邊：$b = \text{distance}(\text{垂足}, P_1)$  
- 斜邊：$c = \text{distance}(P, P_1)$
